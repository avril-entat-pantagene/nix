  ...
  outputs =
    ...
    flake-utils.lib.eachSystem [ "x86_64-linux" ] (
      ...
      let
        ...
        # GDB cross-compiled for ARM targets, replaces the bundled arm-none-eabi-gdb
        # which requires libncurses5 not available on modern distros.
        # See: https://community.st.com/t5/stm32cubeide-mcus/could-not-determine-gdb-version/td-p/734073
        arm-gdb = pkgs.pkgsCross.arm-embedded.buildPackages.gdb;
        ...
        stm32cubeide = pkgs.stdenv.mkDerivation rec {
          ...
          postFixup = ''
            # Replace bundled arm-none-eabi-gdb with system gdb-multiarch
            # The bundled GDB requires libncurses5 which is not available on modern distros.
            # See: https://community.st.com/t5/stm32cubeide-mcus/could-not-determine-gdb-version/td-p/734073
            GNU_TOOLS_DIR=$(find $out/opt/stm32cubeide/plugins -type d -name "com.st.stm32cube.ide.mcu.externaltools.gnu-tools-for-stm32.*.linux64_*" | head -1)
            if [ -n "$GNU_TOOLS_DIR" ] && [ -f "$GNU_TOOLS_DIR/tools/bin/arm-none-eabi-gdb" ]; then
              mv "$GNU_TOOLS_DIR/tools/bin/arm-none-eabi-gdb" "$GNU_TOOLS_DIR/tools/bin/arm-none-eabi-gdb.bak"
              ln -s ${arm-gdb}/bin/arm-none-eabi-gdb "$GNU_TOOLS_DIR/tools/bin/arm-none-eabi-gdb"
            fi
        ...
        stm32cubeide-fhs = pkgs.buildFHSEnv {
          ...
          # Propagate .desktop file and icon so KDE (and other DEs) can find the app
          extraInstallCommands = ''
            mkdir -p $out/share/applications $out/share/pixmaps
            cp ${stm32cubeide}/share/applications/stm32cubeide.desktop $out/share/applications/stm32cubeide.desktop
            substituteInPlace $out/share/applications/stm32cubeide.desktop \
              --replace-fail "Exec=${stm32cubeide}/bin/stm32cubeide" "Exec=$out/bin/stm32cubeide"
            if [ -f ${stm32cubeide}/share/pixmaps/stm32cubeide.xpm ]; then
              cp ${stm32cubeide}/share/pixmaps/stm32cubeide.xpm $out/share/pixmaps/stm32cubeide.xpm
            fi
          '';
        };
